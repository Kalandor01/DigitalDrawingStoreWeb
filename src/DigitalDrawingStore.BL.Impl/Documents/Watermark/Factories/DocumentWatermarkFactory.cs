...
namespace XperiCad.DigitalDrawingStore.BL.Impl.Documents.Watermark.Factories
{
    public class DocumentWatermarkFactory : IDocumentWatermarkFactory
    {
        ...
        {
            ValidateInput(fontSize, opacity, rotation);

            return new DocumentWatermark(text, fontSize, opacity, rotation, 0, 0, verticalPosition, horizontalPosition);
        }
        ...
        private static void ValidateInput(int fontSize, float opacity, int rotation)
        {
